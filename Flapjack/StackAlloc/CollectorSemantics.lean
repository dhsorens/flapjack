...
theorem stackFrameMemcpyStep_register_zero_eq [NeZero width]
  ...
  simp [stackFrameMemcpyStep, stackFrameWriteRegister,
    wordStackMachineWriteRegister, wordStackMachineWriteMemory,
    wordStackMachineBinOp, Ne.symm hscratch0]
...
theorem stackFrameMemcpyStep_register_one [NeZero width]
  ...
  simp [stackFrameMemcpyStep, stackFrameWriteRegister,
    wordStackMachineWriteRegister, wordStackMachineWriteMemory,
    wordStackMachineBinOp, Ne.symm hscratch1]
...
theorem stackFrameMemcpyStep_register_source [NeZero width]
  ...
  simp [stackFrameMemcpyStep, stackFrameWriteRegister,
    wordStackMachineWriteRegister, wordStackMachineWriteMemory,
    wordStackMachineBinOp, Ne.symm hscratch2]
...
theorem stackFrameMemcpyStep_register_destination [NeZero width]
  ...
  simp [stackFrameMemcpyStep, stackFrameWriteRegister,
    wordStackMachineWriteRegister, wordStackMachineWriteMemory,
    wordStackMachineBinOp, Ne.symm hscratch3]
...
theorem stackFrameMemcpyIter_register_source [NeZero width]
  ...
  | succ words ih =>
      ...
      rw [ih]
      rw [stackFrameMemcpyStep_register_source config state hscratch2]
      simp only [wordStackMachineBinOp, Nat.succ_mul, 
        BitVec.ofNat_add]
      rw [BitVec.add_assoc]
      congr 1
...
theorem evalStackFrameFuel_stackGcMoveList_immediate_one [NeZero width]
  ...
  have hadd : evalStackFrameFuel (fuel + 13) afterStore
    ...
    simp [stackGcAddBytes, stackGcAddImmediate, stackGcConst, stackGcAdd,
      ...
      afterScratch, afterStore, Ne.symm hscratch8]
...
theorem evalStackFrameFuel_stackGcMoveList_immediate_body [NeZero width]
    (config : StackGcConfig) (fuel : Nat)
    (state : StackFrameMachineState width)
    (hscratch5 : config.immediateScratch ≠ 5)
    (hscratch7 : config.immediateScratch ≠ 7)
    ...
    (hvalue : state.machine.memory (state.machine.registers 8) &&&
      BitVec.ofNat width 1 = 0) :
    evalStackFrameFuel (fuel + 17) state
        (stackSeq [
          .inst (.mem .load 5 8),
          stackGcSubOne config 7,
          stackGcMoveCode config,
  ...
  have hbody : evalStackFrameFuel (fuel + 17) state
    ...
    simpa [Nat.add_assoc] using hadd
  simpa [stackGcMoveListImmediateState, afterFinal, afterScratch,
    afterStore, afterCount, afterLoad] using hbody
...
theorem evalStackFrameFuel_stackGcMoveList_immediate_one_matches_nat
  ...
  simp [stackFrameNormalRegisterNat, heval,
    ...
    wordStackMachineWriteRegister, wordStackMachineWriteMemory,
    wordStackMachineBinOp, haddress, hmemory', 
    Ne.symm hscratch5]
...
theorem evalStackFrameFuel_stackGcMoveList_immediate_one_count_matches_nat
  ...
  simp [stackFrameNormalRegisterNat, heval,
    stackGcMoveListImmediateState, stackFrameWriteRegister,
    wordStackMachineWriteRegister, wordStackMachineWriteMemory,
    wordStackMachineBinOp, hone, haddress, Ne.symm hscratch7]
...
theorem evalStackFrameFuel_stackGcMoveList_immediate_one_memory_matches_nat
  ...
  simp [stackFrameNormalMemoryNat, heval,
    stackGcMoveListImmediateState, stackFrameWriteRegister,
    wordStackMachineWriteRegister, wordStackMachineWriteMemory,
    wordStackMachineBinOp, haddress, hmemWord, 
    hscratch5', Ne.symm hscratch8,
...
theorem evalStackFrameFuel_stackGcMoveList_immediate_one_memory_relation
  ...
  · subst target
    simp [stackFrameNormalMemoryNat, stackGcMoveListImmediateState,
      ...
      hmemoryAddress, Ne.symm hscratch5,
      Ne.symm hscratch8]
  · have htargetWord : BitVec.ofNat width target ≠
    ...
    simp [stackFrameNormalMemoryNat, stackGcMoveListImmediateState,
      stackFrameWriteRegister, wordStackMachineWriteRegister,
      wordStackMachineWriteMemory, wordStackMachineBinOp, haddress,
      htargetOfNat, htargetAddress, hmemory target htarget,
...
theorem evalStackFrameFuel_stackGcMoveList_forwarding_one [NeZero width]
  ...
  have hsub : evalStackFrameFuel (fuel + 36) afterLoad
    ...
    simp [afterCount, afterLoad, stackGcMoveListAfterCount,
      stackGcSubOne, stackGcSubImmediate, 
      stackGcConst, stackGcSub, stackSeq, evalStackFrameFuel,
      evalStackFrameFuelWithCode, stackFrameBasic, stackFrameWriteRegister,
  ...
  have hcondition' :
      stackMachineCondition afterFinal.machine .notEqual 7 (.imm 0) = false := by
    simp [stackMachineCondition, afterFinal, afterScratch, afterStore,
      afterMove, afterCount, stackGcMoveListAfterCount, 
...
theorem evalStackFrameFuel_stackGcMoveList_forwarding_body [NeZero width]
    (config : StackGcConfig) (fuel : Nat)
    (state : StackFrameMachineState width)
    (hscratch0 : config.immediateScratch ≠ 0)
    (hscratch1 : config.immediateScratch ≠ 1)
    ...
    (hstoreDomain :
      (stackGcMoveForwardingState config
        (stackGcMoveListAfterCount config state)).memoryDomain
          ((stackGcMoveForwardingState config
            (stackGcMoveListAfterCount config state)).machine.registers 8) = true) :
    evalStackFrameFuel (fuel + 38) state
      (stackSeq [
        .inst (.mem .load 5 8),
        stackGcSubOne config 7,
        stackGcMoveCode config,
...
theorem evalStackFrameFuel_stackGcMoveList_forwarding_body_with_nat_relation
    ...
    evalStackFrameFuel (fuel + 38) state
        (stackSeq [
          ...
          .inst (.mem .store 5 8),
        stackGcAddBytes config 8]) =
        some (.normal (stackGcMoveListForwardingState config state)) ∧
      ((stackGcMoveListForwardingState config state).machine.registers 8).toNat =
  ...
  have hmemoryAddress := hmemory scan hscanBound
  have hmemoryReg8 :
      (state.machine.memory (state.machine.registers 8)).toNat =
        memory scan := by
  ...
  have heval := evalStackFrameFuel_stackGcMoveList_forwarding_body config fuel
    state hscratch0 hscratch1 hscratch5 hscratch7 hscratch8 hdomain hodd
    hheaderDomain hforward hstoreDomain
  have hafterMove5 :
      (stackGcMoveForwardingState config
        (stackGcMoveListAfterCount config state)).machine.registers 5 =
  ...
  have hscanOut :
    ...
    simp [stackGcMoveListForwardingState, stackFrameWriteRegister,
      wordStackMachineWriteRegister, wordStackMachineWriteMemory,
      wordStackMachineBinOp, haddress, hafterMove8, 
      BitVec.toNat_add, BitVec.toNat_ofNat,
      Nat.mod_eq_of_lt hnextBound, 
      Ne.symm hscratch8]
  have hmemoryOut : ∀ target, target < 2 ^ width →
    ...
    · have htargetWord : BitVec.ofNat width target ≠
      ...
      have htargetOfNat : BitVec.ofNat width target ≠
        ...
        simpa [BitVec.toNat_ofNat, Nat.mod_eq_of_lt htarget,
          Nat.mod_eq_of_lt hscanBound] using hto
      simp [stackGcMoveListForwardingState, stackGcMoveForwardingState,
        stackGcMoveForwardingSuffixState, stackGcMoveAddressState,
        stackGcMoveListAfterCount, stackFrameWriteRegister,
        wordStackMachineWriteRegister, wordStackMachineWriteMemory,
        wordStackMachineBinOp, haddress, htargetOfNat,
        htargetAddress, hmemory target htarget, 
        Ne.symm hscratch0,
        Ne.symm hscratch1, Ne.symm hscratch5, 
        Ne.symm hscratch8]
...
theorem evalStackFrameFuel_stackGcMoveList_forwarding_one_memory_relation
  ...
  by_cases htargetAddress : target = address
  · subst target
    simp only [stackFrameNormalMemoryNat]
    rw [hfinalMemory, hfinal5, hafterMove5, hforwardValue]
    simp
  · have htargetWord : BitVec.ofNat width target ≠
        state.machine.registers 8 := by
    ...
    simp [stackFrameNormalMemoryNat, stackGcMoveListForwardingState,
      ...
      haddress, htargetOfNat, htargetAddress,
      hmemory target htarget, 
...
theorem evalStackFrameFuel_stackGcMoveList_copy_one [NeZero width]
  ...
  have haddress7 (source : StackFrameMachineState width) :
    ...
    simp [stackGcMoveAddressState, stackFrameWriteRegister,
      ...
      Ne.symm hscratch0, Ne.symm hscratch1, Ne.symm hscratch7]
  ...
  have hcount7 : afterCount.machine.registers 7 = BitVec.ofNat width 0 := by
    simp [afterCount, stackGcMoveListAfterCount, 
      stackFrameWriteRegister, wordStackMachineWriteRegister,
      wordStackMachineBinOp, hone]
  ...
  have hcondition' :
      stackMachineCondition afterFinal.machine .notEqual 7 (.imm 0) = false := by
    simp [stackMachineCondition, hfinal7, hmove7, hcount7, 
      ]
...
theorem evalStackFrameFuel_stackGcMoveList_copy_body [NeZero width]
    (config : StackGcConfig) (fuel : Nat)
    (state : StackFrameMachineState width)
    (hscratch0 : config.immediateScratch ≠ 0)
    (hscratch1 : config.immediateScratch ≠ 1)
...
theorem evalStackFrameFuel_stackGcMoveLoop_code_step_scan_matches_nat
  ...
  have hreg :
    ...
    simp [stackGcMoveLoopCodeStepState, stackFrameWriteRegister,
      wordStackMachineWriteRegister, wordStackMachineBinOp,
      wordStackMachineShift, Ne.symm hscratch8]
  rw [hreg, BitVec.toNat_add]
  rw [hcount]
  simp [haddress, BitVec.toNat_ofNat,
...
theorem evalStackFrameFuel_stackGcMoveRootsBitmaps_iterate [NeZero width]
    (config : StackGcConfig) (fuel stepFuel iterations : Nat)
    (state : StackFrameMachineState width)
    (step : StackFrameMachineState width → StackFrameMachineState width)
    (hcondition : ∀ current, current < iterations →
      stackMachineCondition
        (stackFrameIterate step current state).machine
          .notTest 9 (.reg 9) = true)
    (hbody : ∀ current extra, current < iterations →
      evalStackFrameFuel (extra + stepFuel)
        (stackFrameIterate step current state)
        (stackSeq [
    ...
    (hfinal : stackMachineCondition
        (stackFrameIterate step iterations state).machine
          .notTest 9 (.reg 9) = false) :
    evalStackFrameFuel (fuel + iterations + stepFuel + 3) state
        (stackGcMoveRootsBitmapsCode config) =
      some (.normal (stackFrameIterate step iterations state)) := by
  simpa [stackGcMoveRootsBitmapsCode, stackGcWhile] using
    (evalStackFrameFuel_loop_iterate fuel stepFuel iterations state
      .notTest 9 (.reg 9)
      (stackSeq [
        stackGcMove 0 9,
...
theorem evalStackFrameFuel_stackGcMoveLoop_data_branch [NeZero width]
  ...
  have hshift :
    ...
    simp [afterShift, afterLength, stackGcShiftImmediate,
      stackGcConst, stackSeq, evalStackFrameFuel,
      evalStackFrameFuelWithCode, stackFrameBasic, stackFrameWriteRegister,
      wordStackMachineWriteRegister, wordStackMachineShift, 
      Ne.symm hscratch7]
  have hadd :
    ...
    simp [stackGcMoveLoopCodePrefixState, afterShift, afterLength,
      ...
      wordStackMachineBinOp, Ne.symm hscratch8]
...
theorem evalStackFrameFuel_stackGcMoveLoop_data_step_with_nat_relation
    ...
    (hmove :
      ...
      some (.normal final))
    (hrelation :
      stackGcMachineNatRelation final
        (stackGcNatMoveList config
          (stackGcNatDecodeLength config (memory scan))
          (scan + config.bytesInWord) index destination oldBase memory domain).nextScan
        (stackGcNatMoveList config
          (stackGcNatDecodeLength config (memory scan))
          (scan + config.bytesInWord) index destination oldBase memory domain).memory) :
    let moved := stackGcNatMoveList config
      (stackGcNatDecodeLength config (memory scan))
      (scan + config.bytesInWord) index destination oldBase memory domain
    evalStackFrameFuel (fuel + 16) state
        (stackSeq [
          ...
          .ite .test 7 (.imm 4)
            ...
            (stackSeq [
              ...
              stackGcAdd 8 7])]) =
        some (.normal final) ∧
      stackGcMachineNatRelation final moved.nextScan moved.memory := by
  dsimp
  have hconditionIff := stackGcMoveLoop_header_condition_matches_nat
    state scan memory hwidth haddress hmemory
  have hcondition := hconditionIff.mpr hcodeNat
...
theorem evalStackFrameFuel_stackGcMoveLoop_code_step_with_nat_relation
    [NeZero width] (config : StackGcConfig) (fuel scan : Nat)
    (state : StackFrameMachineState width)
    (memory : Nat → Nat) (domain : Nat → Bool)
    (hwidth : 3 ≤ width)
    (hscratch7 : config.immediateScratch ≠ 7)
  ...
  have hcode : stackMachineCondition
    ...
    exact (hconditionIff.mp htrue) hcodeNat
  constructor
  · exact evalStackFrameFuel_stackGcMoveLoop_code_step config fuel state
      hscratch7 hscratch8 hdomain hcode
  · exact stackGcMoveLoopCodeStepState_preserves_nat_relation_of_memory
...
theorem evalStackFrameFuel_stackGcMoveList_immediate_body_with_nat_relation
    ...
    evalStackFrameFuel (fuel + 17) state
        (stackSeq [
          ...
          .inst (.mem .store 5 8),
          stackGcAddBytes config 8]) =
        some (.normal (stackGcMoveListImmediateState config state)) ∧
      stackGcMachineNatRelation
        (stackGcMoveListImmediateState config state)
        (scan + config.bytesInWord)
        (fun current => if current = scan then memory scan else memory current) := by
  have hscanBound : scan < 2 ^ width := by
    rw [← hrelation.1]
    exact (state.machine.registers 8).isLt
  have hmemoryAt :
      (state.machine.memory (state.machine.registers 8)).toNat =
        memory scan := by
    simpa [haddress] using hrelation.2 scan hscanBound
  have heval := evalStackFrameFuel_stackGcMoveList_immediate_body config fuel
    state hscratch5 hscratch7 hscratch8 hdomain hbit
  have hbytes : config.bytesInWord < 2 ^ width := by omega
  have hscanOut :
      ((stackGcMoveListImmediateState config state).machine.registers 8).toNat =
        scan + config.bytesInWord := by
    simp [stackGcMoveListImmediateState, stackFrameWriteRegister,
      wordStackMachineWriteRegister, wordStackMachineWriteMemory,
      wordStackMachineBinOp, haddress, 
      BitVec.toNat_add, BitVec.toNat_ofNat,
      Nat.mod_eq_of_lt hnextBound, 
      Ne.symm hscratch5, Ne.symm hscratch8]
  have hmemoryOut : stackGcMachineMemoryMatchesNat
      (stackGcMoveListImmediateState config state)
      (fun current => if current = scan then memory scan else memory current) := by
    intro target htarget
    by_cases htargetAddress : target = scan
    · subst target
      simp [stackGcMoveListImmediateState, stackFrameWriteRegister,
        ...
        Ne.symm hscratch5, 
        Ne.symm hscratch8]
      simpa [haddress] using hmemoryAt
    · have htargetWord : BitVec.ofNat width target ≠
          state.machine.registers 8 := by
        rw [haddress]
        intro heq
      ...
      simp [stackGcMoveListImmediateState, stackFrameWriteRegister,
        wordStackMachineWriteRegister, wordStackMachineWriteMemory,
        wordStackMachineBinOp, haddress, htargetAddress,
        htargetOfNat,
        hrelation.2 target htarget, 
        Ne.symm hscratch5, Ne.symm hscratch8]
  exact ⟨heval, ⟨hscanOut, hmemoryOut⟩⟩
...
theorem evalStackFrameFuel_stackGcMoveList_copy_body_with_nat_relation
  ...
  have hfinalMemory :
    ...
    simp only [stackFrameWriteRegister, wordStackMachineWriteRegister,
      ...
      Ne.symm hscratch5,
      Ne.symm hscratch8]
    simp only [if_neg (show (5 : Nat) ≠ 8 by decide),
      if_neg (show ¬ False by decide)]
    rfl
  have hfinalValue :
      ((stackGcMoveListCopyState config state).machine.memory
  ...
  have hscanOut :
    ...
    simp [stackGcMoveListCopyState, stackFrameWriteRegister,
      wordStackMachineWriteRegister, wordStackMachineWriteMemory,
      wordStackMachineBinOp, haddress, hcopyAddress, 
      BitVec.toNat_add, BitVec.toNat_ofNat,
      Nat.mod_eq_of_lt hnextBound, 
      Ne.symm hscratch8]
  have hmemoryOut : ∀ target, target < 2 ^ width →
...
theorem evalStackFrameFuel_stackGcMoveCopySuffixAfterMemcpy [NeZero width]
  ...
  simp [stackGcMoveCopySuffixAfterMemcpy,
    ...
    hdomain, 
    Ne.symm hscratch0, Ne.symm hscratch1,
    Ne.symm hscratch2, Ne.symm hscratch4,
    Ne.symm hscratch5, Ne.symm hscratch6]
...
theorem evalStackFrameFuel_stackGcMoveCode_copy_iter [NeZero width]
  ...
  simp [stackGcMoveCode, stackSeq, evalStackFrameFuel,
    evalStackFrameFuelWithCode, houter, hinner, 
    hprefix'', hcopyPrefix'', 
    hcopySuffix'', Nat.add_assoc]

theorem evalStackFrameFuel_stackGcMoveCopySuffixAfterMemcpy_with_nat_relation
    ...
    (hscratch6 : config.immediateScratch ≠ 6)
    (hdomain : ∀ address, state.memoryDomain address = true)
    (hmemory : ∀ target, target < 2 ^ width →
      (state.machine.memory (BitVec.ofNat width target)).toNat =
        (stackGcNatMemcpy config words source destination memory domain).memory target)
    (hwriteAddress :
      (stackGcMoveCopySuffixStateAfterMemcpy config state).machine.registers 2 =
        BitVec.ofNat width source)
    (hwriteValue :
      ((stackGcMoveCopySuffixStateAfterMemcpy config state).machine.registers 0).toNat =
        index * 4)
    (hfinalDestination :
      ((stackGcMoveCopySuffixStateAfterMemcpy config state).machine.registers 3).toNat =
        destination + words * config.bytesInWord)
    (hfinalValue :
      ((stackGcMoveCopySuffixStateAfterMemcpy config state).machine.registers 5).toNat =
        stackGcNatUpdateAddress config index value)
    (hfinalIndex :
      ((stackGcMoveCopySuffixStateAfterMemcpy config state).machine.registers 4).toNat =
        index + words)
    (hsourceBound : source < 2 ^ width)
    :
    evalStackFrameFuel (fuel + 24) state
        (stackGcMoveCopySuffixAfterMemcpy config) =
      some (.normal (stackGcMoveCopySuffixStateAfterMemcpy config state)) ∧
      ((stackGcMoveCopySuffixStateAfterMemcpy config state).machine.registers 3).toNat =
      ...
      ∀ target, target < 2 ^ width →
        ((stackGcMoveCopySuffixStateAfterMemcpy config state).machine.memory
          (BitVec.ofNat width target)).toNat =
          (stackGcNatMoveCopySuffix config words source destination index value
            memory domain).memory target := by
  have heval := evalStackFrameFuel_stackGcMoveCopySuffixAfterMemcpy config fuel
    state hscratch0 hscratch1 hscratch2 hscratch3 hscratch4 hscratch5 hscratch6
    hdomain
  have hfinalMemory (address : Word width) :
      (stackGcMoveCopySuffixStateAfterMemcpy config state).machine.memory address =
        if address = (stackGcMoveCopySuffixStateAfterMemcpy config state).machine.registers 2 then
          (stackGcMoveCopySuffixStateAfterMemcpy config state).machine.registers 0
        else state.machine.memory address := by
    simp [stackGcMoveCopySuffixStateAfterMemcpy, stackFrameWriteRegister,
      wordStackMachineWriteRegister, wordStackMachineWriteMemory,
      wordStackMachineBinOp, wordStackMachineShift, 
      
      Ne.symm hscratch0, Ne.symm hscratch1, Ne.symm hscratch2,
      Ne.symm hscratch4, Ne.symm hscratch5,
      Ne.symm hscratch6]
  refine ⟨heval, ?_, ?_, ?_, ?_⟩
  · rw [stackGcNatMoveCopySuffix_nextAddress]
    exact hfinalDestination
  · simpa [stackGcNatMoveCopySuffix_nextIndex] using hfinalIndex
  · simpa [stackGcNatMoveCopySuffix_value] using hfinalValue
  · intro target htarget
    by_cases htargetAddress : target = source
    · subst target
      rw [hfinalMemory, hwriteAddress]
      simp only []
      simpa [stackGcNatMoveCopySuffix] using hwriteValue
    · have htargetWord : BitVec.ofNat width target ≠
          (stackGcMoveCopySuffixStateAfterMemcpy config state).machine.registers 2 := by
        rw [hwriteAddress]
        intro heq
        have hto := congrArg BitVec.toNat heq
        apply htargetAddress
        simpa [BitVec.toNat_ofNat, Nat.mod_eq_of_lt htarget,
          Nat.mod_eq_of_lt hsourceBound] using hto
      rw [hfinalMemory, if_neg htargetWord]
      rw [hmemory target htarget]
      simp [stackGcNatMoveCopySuffix, htargetAddress]

theorem evalStackFrameFuel_stackGcMoveCopySuffix_iter_with_nat_relation
    [NeZero width] (config : StackGcConfig) (fuel words : Nat)
    (source destination index value : Nat)
    (state : StackFrameMachineState width)
    (memory : Nat → Nat) (domain : Nat → Bool)
    ...
    (hwriteValue :
      ((stackGcMoveCopySuffixStateAfterMemcpy config
        (stackFrameMemcpyIter config words state)).machine.registers 0).toNat =
        index * 4)
    (hfinalDestination :
      ((stackGcMoveCopySuffixStateAfterMemcpy config
        (stackFrameMemcpyIter config words state)).machine.registers 3).toNat =
    ...
    (hfinalValue :
      ((stackGcMoveCopySuffixStateAfterMemcpy config
        (stackFrameMemcpyIter config words state)).machine.registers 5).toNat =
        stackGcNatUpdateAddress config index value)
    (hfinalIndex :
      ((stackGcMoveCopySuffixStateAfterMemcpy config
        (stackFrameMemcpyIter config words state)).machine.registers 4).toNat =
        index + words)
    (hsourceBound : source < 2 ^ width) :
    evalStackFrameFuel (fuel + words + 25) state
        (stackGcMoveCopySuffix config) =
      some (.normal (stackGcMoveCopySuffixIterState config words state)) ∧
      ((stackGcMoveCopySuffixIterState config words state).machine.registers 3).toNat =
        (stackGcNatMoveCopySuffix config words source destination index value
          memory domain).nextAddress ∧
      ((stackGcMoveCopySuffixIterState config words state).machine.registers 4).toNat =
        (stackGcNatMoveCopySuffix config words source destination index value
          memory domain).nextIndex ∧
      ((stackGcMoveCopySuffixIterState config words state).machine.registers 5).toNat =
        (stackGcNatMoveCopySuffix config words source destination index value
          memory domain).value ∧
      ∀ target, target < 2 ^ width →
        ((stackGcMoveCopySuffixIterState config words state).machine.memory
          (BitVec.ofNat width target)).toNat =
          (stackGcNatMoveCopySuffix config words source destination index value
            memory domain).memory target := by
  have hmemcpy := evalStackFrameFuel_stackGcMemcpy_matches_nat config fuel words
    state source destination memory hstate hmemory hsource hdestination hbound
    hscratch0 hscratch1 hscratch2 hscratch3 hdomain
  have hmemory' : ∀ target, target < 2 ^ width →
      ((stackFrameMemcpyIter config words state).machine.memory
        (BitVec.ofNat width target)).toNat =
        (stackGcNatMemcpy config words source destination memory domain).memory target := by
    intro target htarget
    have htarget' := hmemcpy.2.2 (BitVec.ofNat width target)
    have hdomainIrrel := stackGcNatMemcpy_memory_domain_irrel config words
      source destination memory domain (fun _ => true)
    rw [hdomainIrrel]
    simpa [BitVec.toNat_ofNat, Nat.mod_eq_of_lt htarget] using htarget'
  have hdomain' : ∀ address,
      (stackFrameMemcpyIter config words state).memoryDomain address = true := by
    have hdomainEq :
        (stackFrameMemcpyIter config words state).memoryDomain =
          state.memoryDomain :=
      stackFrameMemcpyIter_memoryDomain config words state
    intro address
    rw [hdomainEq]
    exact hdomain address
  have htail :=
    evalStackFrameFuel_stackGcMoveCopySuffixAfterMemcpy_with_nat_relation
      config (fuel + words) words source destination index value
      (stackFrameMemcpyIter config words state) memory domain
      hscratch0 hscratch1 hscratch2 hscratch3 hscratch4 hscratch5 hscratch6
      hdomain' hmemory' hwriteAddress hwriteValue hfinalDestination
      hfinalValue hfinalIndex hsourceBound
  have hseq := evalStackFrameFuel_seq_normal (fuel + words + 24) state
    (stackFrameMemcpyIter config words state)
    (stackGcMemcpy config) (stackGcMoveCopySuffixAfterMemcpy config) hmemcpy.1
...
theorem evalStackFrameFuel_stackGcMoveList_copy_body_iter_with_nat_relation
    [NeZero width] (config : StackGcConfig) (fuel words : Nat)
    (state : StackFrameMachineState width)
    (scan source destination index value : Nat)
    (memory : Nat → Nat) (domain : Nat → Bool)
    (hscratch0 : config.immediateScratch ≠ 0)
    (hscratch1 : config.immediateScratch ≠ 1)
    (hscratch2 : config.immediateScratch ≠ 2)
    ...
    (hstate : StackFrameMemcpyCorrespondence
      (stackGcMoveCopyPrefixState config
        (stackGcMoveAddressState config
          (stackGcMoveListAfterCount config state))) words
      (BitVec.ofNat width source) (BitVec.ofNat width destination)
      (fun current => BitVec.ofNat width (memory current.toNat)))
    (hmemoryBound : ∀ address, memory address < 2 ^ width)
    (hsource : ∀ offset, offset ≤ words →
      source + offset * config.bytesInWord < 2 ^ width)
    (hdestination : ∀ offset, offset ≤ words →
      destination + offset * config.bytesInWord < 2 ^ width)
    (hwriteAddress :
      (stackGcMoveCopySuffixStateAfterMemcpy config
        (stackFrameMemcpyIter config words
          (stackGcMoveCopyPrefixState config
            (stackGcMoveAddressState config
              (stackGcMoveListAfterCount config state))))).machine.registers 2 =
        BitVec.ofNat width source)
    (hwriteValue :
      ((stackGcMoveCopySuffixStateAfterMemcpy config
        (stackFrameMemcpyIter config words
          (stackGcMoveCopyPrefixState config
            (stackGcMoveAddressState config
              (stackGcMoveListAfterCount config state))))).machine.registers 0).toNat =
        index * 4)
    (hfinalDestination :
      ((stackGcMoveCopySuffixStateAfterMemcpy config
        (stackFrameMemcpyIter config words
          (stackGcMoveCopyPrefixState config
            (stackGcMoveAddressState config
              (stackGcMoveListAfterCount config state))))).machine.registers 3).toNat =
        destination + words * config.bytesInWord)
    (hfinalValue :
      ((stackGcMoveCopySuffixStateAfterMemcpy config
        (stackFrameMemcpyIter config words
    ...
    (hfinalIndex :
      ((stackGcMoveCopySuffixStateAfterMemcpy config
        (stackFrameMemcpyIter config words
          (stackGcMoveCopyPrefixState config
            (stackGcMoveAddressState config
              (stackGcMoveListAfterCount config state))))).machine.registers 4).toNat =
        index + words)
    (hsourceBound : source < 2 ^ width)
    (hcopyAddress :
      (stackGcMoveCopySuffixIterState config words
    ...
    evalStackFrameFuel (fuel + words + 44) state
        (stackSeq [
          ...
          .inst (.mem .store 5 8),
          stackGcAddBytes config 8]) =
      some (.normal (stackGcMoveListCopyIterState config words state)) ∧
      ((stackGcMoveListCopyIterState config words state).machine.registers 8).toNat =
        (stackGcNatMoveListCopyBody config words scan source destination index value
          memory domain).nextScan ∧
      ((stackGcMoveListCopyIterState config words state).machine.registers 4).toNat =
        (stackGcNatMoveListCopyBody config words scan source destination index value
          memory domain).nextIndex ∧
      ((stackGcMoveListCopyIterState config words state).machine.registers 3).toNat =
        (stackGcNatMoveListCopyBody config words scan source destination index value
          memory domain).nextAddress ∧
      ((stackGcMoveListCopyIterState config words state).machine.registers 5).toNat =
        (stackGcNatMoveListCopyBody config words scan source destination index value
          memory domain).memory scan ∧
      ∀ target, target < 2 ^ width →
        ((stackGcMoveListCopyIterState config words state).machine.memory
          (BitVec.ofNat width target)).toNat =
          (stackGcNatMoveListCopyBody config words scan source destination index value
            memory domain).memory target := by
  let afterCount := stackGcMoveListAfterCount config state
  ...
  let afterFinal := stackFrameWriteRegister afterScratch 8
    (wordStackMachineBinOp .add (afterScratch.machine.registers 8)
      (BitVec.ofNat width config.bytesInWord))
  have hmoveAddress : afterMove.machine.registers 8 = BitVec.ofNat width scan := by
    dsimp [afterMove, afterCount]
    rw [hcopyAddress, haddress]
  have hmove := evalStackFrameFuel_stackGcMoveCode_copy_iter_with_nat_relation
    config (fuel + 12) words afterCount source destination index value memory domain
    hscratch0 hscratch1 hscratch2 hscratch3 hscratch4 hscratch5 hscratch6 hodd
    hheaderDomain hnotforwarding hcopyCount hbound hcopyDomain hstate hmemoryBound
    hsource hdestination hwriteAddress hwriteValue hfinalDestination hfinalValue
    hfinalIndex hsourceBound
  have heval := evalStackFrameFuel_stackGcMoveList_copy_body_iter config fuel words
    state hscratch0 hscratch1 hscratch2 hscratch3 hscratch4 hscratch5 hscratch6
    hscratch7 hscratch8 hdomain hodd hheaderDomain hnotforwarding hcopyCount hbound
    hstoreDomain
  refine ⟨heval, ?_, ?_, ?_, ?_, ?_⟩
  · change (afterFinal.machine.registers 8).toNat =
      (stackGcNatMoveListCopyBody config words scan source destination index value
        memory domain).nextScan
    have hbytes : config.bytesInWord < 2 ^ width := by omega
    simp [afterFinal, afterScratch, afterStore,
      stackFrameWriteRegister, wordStackMachineWriteRegister,
      wordStackMachineWriteMemory, wordStackMachineBinOp, 
      hmoveAddress, BitVec.toNat_add,
      BitVec.toNat_ofNat, Nat.mod_eq_of_lt hnextBound, 
      
      Ne.symm hscratch8, stackGcNatMoveListCopyBody]
  · change (afterFinal.machine.registers 4).toNat =
      (stackGcNatMoveListCopyBody config words scan source destination index value
        memory domain).nextIndex
    simpa [afterFinal, afterScratch, afterStore, afterMove,
      stackGcNatMoveListCopyBody, stackFrameWriteRegister,
      wordStackMachineWriteRegister, wordStackMachineWriteMemory,
      wordStackMachineBinOp,
      ...
      Ne.symm hscratch0, Ne.symm hscratch1, Ne.symm hscratch2,
      Ne.symm hscratch3, Ne.symm hscratch4, Ne.symm hscratch5,
      Ne.symm hscratch6, Ne.symm hscratch7, Ne.symm hscratch8] using hmove.2.2.1
  · change (afterFinal.machine.registers 3).toNat =
      (stackGcNatMoveListCopyBody config words scan source destination index value
        memory domain).nextAddress
    simpa [afterFinal, afterScratch, afterStore, afterMove,
      stackGcNatMoveListCopyBody, stackFrameWriteRegister,
      ...
      Ne.symm hscratch0, Ne.symm hscratch1, Ne.symm hscratch2,
      Ne.symm hscratch3, Ne.symm hscratch4, Ne.symm hscratch5,
      Ne.symm hscratch6, Ne.symm hscratch7, Ne.symm hscratch8] using hmove.2.1
  · change (afterFinal.machine.registers 5).toNat =
      (stackGcNatMoveListCopyBody config words scan source destination index value
        memory domain).memory scan
    simpa [afterFinal, afterScratch, afterStore, afterMove,
      stackGcNatMoveListCopyBody, stackFrameWriteRegister,
      ...
      Ne.symm hscratch0, Ne.symm hscratch1, Ne.symm hscratch2,
      Ne.symm hscratch3, Ne.symm hscratch4, Ne.symm hscratch5,
      Ne.symm hscratch6, Ne.symm hscratch7, Ne.symm hscratch8] using hmove.2.2.2.1
  · intro target htarget
    change (afterFinal.machine.memory (BitVec.ofNat width target)).toNat =
      (stackGcNatMoveListCopyBody config words scan source destination index value
        memory domain).memory target
    by_cases htargetScan : target = scan
    · subst target
      have hvalue' := hmove.2.2.2.1
      simp [afterFinal, afterScratch, afterStore, afterMove,
        ...
        Ne.symm hscratch8]
    · have hcopy := hmove.2.2.2.2 target htarget
      have htargetWord : BitVec.ofNat width target ≠ afterMove.machine.registers 8 := by
        rw [hcopyAddress, haddress]
        intro heq
        have hto := congrArg BitVec.toNat heq
        apply htargetScan
...
theorem evalStackFrameFuel_stackGcMoveLoop_code_step_with_state_relation
    [NeZero width] (config : StackGcConfig) (fuel : Nat)
    (state : StackFrameMachineState width)
    (scan index destination : Nat) (memory : Nat → Nat)
    (domain : Nat → Bool)
  ...
  have hmemory :
      (state.machine.memory (state.machine.registers 8)).toNat =
        memory scan := by
    have h := hbase.2 scan hscanBound
    simpa [haddress] using h
  have heval := evalStackFrameFuel_stackGcMoveLoop_code_step_with_nat_relation
    config fuel scan state memory domain hwidth hscratch7 hscratch8 hdomain
    haddress hmemory hbase hcodeNat hadvance hcount
  have hpreserve := stackGcMoveLoopCodeStepState_preserves_nat_state_relation
    config fuel state scan index destination memory domain hwidth hscratch3
    hscratch4 hscratch7 hscratch8 hdomain haddress
...
theorem evalStackFrameFuel_stackGcMoveLoop_iterate_with_state_relation
    [NeZero width] (config : StackGcConfig)
    (fuel stepFuel iterations : Nat)
    (state : StackFrameMachineState width)
    (step : StackFrameMachineState width → StackFrameMachineState width)
  ...
  have hrelation :=
    stackGcMachineNatMoveLoopRelation_stackFrameIterate_of_step iterations
      state step scans indices destinations memories hinitial hstep
  have heval := evalStackFrameFuel_stackGcMoveLoop_iterate config fuel
    stepFuel iterations state step hcondition hbody hfinal
  exact ⟨heval, hrelation⟩

theorem evalStackFrameFuel_stackGcMoveLoop_iterate_with_machine_nat_composition
    [NeZero width] (config : StackGcConfig)
    ...
    (hstep : ∀ current, current < iterations →
      ...
      stackGcMachineNatMoveLoopRelation
        ...
        (scans (current + 1)) (indices (current + 1))
        (destinations (current + 1)) (memories (current + 1)))
    (hnatstep : ∀ current remaining, current < iterations →
    ...
    evalStackFrameFuel (fuel + iterations + stepFuel + 3) state
      ...
      stackGcMachineNatMoveLoopRelation
        ...
        (scans iterations) (indices iterations) (destinations iterations)
        (memories iterations) := by
  have hmachine := evalStackFrameFuel_stackGcMoveLoop_iterate_with_state_relation
    config fuel stepFuel iterations state step scans indices destinations memories
    hinitial hcondition hbody hfinal hstep
  have hnat := stackGcNatMoveLoop_iterate_of_step config natFuel iterations oldBase
...
theorem evalStackFrameFuel_stackGcMoveLoop_data_step_with_machine_nat_step
    [NeZero width] (config : StackGcConfig) (fuel scan index destination oldBase : Nat)
    (state final : StackFrameMachineState width)
    (memory : Nat → Nat) (domain : Nat → Bool) (condition : Bool)
    (hwidth : 3 ≤ width)
    ...
    evalStackFrameFuel (fuel + 16) state
      ...
      stackGcMachineNatMoveLoopRelation final moved.nextScan moved.nextIndex
        moved.nextAddress moved.memory := by
  have heval := evalStackFrameFuel_stackGcMoveLoop_data_step_with_state_relation
    config fuel scan index destination oldBase state final memory domain condition
    hwidth hscratch7 hscratch8 hdomain haddress hrelation hscan hcodeNat hmove
    hmovedRelation
  have hnat := stackGcNatMoveLoop_data_step config fuel scan index destination
    oldBase memory domain condition hscan hcodeNat
  exact ⟨heval.1, hnat, heval.2⟩
