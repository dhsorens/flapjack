...
def stackFrameMemcpyIter [NeZero width]
  ...
  | 0, state => state
  | count + 1, state =>
      stackFrameMemcpyIter config count (stackFrameMemcpyStep config state)
...
def stackGcMoveCopySuffixIterState [NeZero width]
    (config : StackGcConfig) (iterations : Nat)
    (state : StackFrameMachineState width) :
    StackFrameMachineState width :=
  let state0 := stackFrameMemcpyIter config (iterations + 1) state
  let state1 := stackFrameWriteRegister state0 0
    (wordStackMachineBinOp .or (state0.machine.registers 6)
      (state0.machine.registers 6))
  let state2 := stackFrameWriteRegister state1 config.immediateScratch
...
def stackGcMoveCopyIterState [NeZero width]
    (config : StackGcConfig) (iterations : Nat)
    (state : StackFrameMachineState width) :
    StackFrameMachineState width :=
  stackGcMoveCopySuffixIterState config iterations
    (stackGcMoveCopyPrefixState config
      (stackGcMoveAddressState config state))
...
def stackGcMoveListOnePreState [NeZero width]
    (config : StackGcConfig) (state : StackFrameMachineState width) :
    StackFrameMachineState width :=
  let state0 := stackFrameWriteRegister state 5
    (state.machine.memory (state.machine.registers 8))
  let state1 := stackFrameWriteRegister state0 config.immediateScratch
    (BitVec.ofNat width 1)
  stackFrameWriteRegister state1 7
    (wordStackMachineBinOp .sub (state1.machine.registers 7)
      (state1.machine.registers config.immediateScratch))

def stackGcMoveListOneState [NeZero width]
    (config : StackGcConfig) (moved : StackFrameMachineState width) :
    StackFrameMachineState width :=
  let state0 := { moved with machine :=
    (wordStackMachineWriteMemory moved.machine
      (moved.machine.registers 8) (moved.machine.registers 5)) }
  let state1 := stackFrameWriteRegister state0 config.immediateScratch
    (BitVec.ofNat width config.bytesInWord)
  stackFrameWriteRegister state1 8
    (wordStackMachineBinOp .add (state1.machine.registers 8)
      (state1.machine.registers config.immediateScratch))
...
theorem stackFrameMemcpyStep_count_succ [NeZero width]
    (config : StackGcConfig) (state : StackFrameMachineState width)
    (count : Nat)
    (hscratch0 : config.immediateScratch ≠ 0)
    (hcount : state.machine.registers 0 = BitVec.ofNat width (count + 1))
    (hbound : count + 1 < 2 ^ width) :
    (stackFrameMemcpyStep config state).machine.registers 0 =
      BitVec.ofNat width count := by
  have hcounter :
      BitVec.ofNat width (count + 1) - BitVec.ofNat width 1 =
        BitVec.ofNat width count := by
    exact BitVec.ofNat_sub_ofNat_of_le (count + 1) 1
      (Nat.one_lt_pow (NeZero.ne width) (by decide)) (by omega)
  simp [stackFrameMemcpyStep, hcount, stackFrameWriteRegister,
    wordStackMachineWriteRegister, wordStackMachineWriteMemory,
    wordStackMachineBinOp, hscratch0, Ne.symm hscratch0, hcounter]
...
theorem evalStackFrameFuel_stackGcMemcpy_iter [NeZero width]
    (config : StackGcConfig) (fuel count : Nat)
    (state : StackFrameMachineState width)
    (hscratch0 : config.immediateScratch ≠ 0)
    (hscratch1 : config.immediateScratch ≠ 1)
    (hscratch2 : config.immediateScratch ≠ 2)
    (hscratch3 : config.immediateScratch ≠ 3)
    (hcount : state.machine.registers 0 = BitVec.ofNat width (count + 1))
    (hcountNonzero : BitVec.ofNat width (count + 1) ≠ 0)
    (hbound : count + 1 < 2 ^ width)
    (hheaderDomain : ∀ address, state.memoryDomain address = true) :
    evalStackFrameFuel (fuel + 24 + 28 * count) state (stackGcMemcpy config) =
      some (.normal (stackFrameMemcpyIter config (count + 1) state)) := by
  induction count generalizing fuel state with
  | zero =>
      simpa [stackFrameMemcpyIter] using
        (evalStackFrameFuel_stackGcMemcpy_one config fuel state
          hscratch0 hscratch1 hscratch2 hscratch3 hcount hheaderDomain)
  | succ count ih =>
      have hstepCount := stackFrameMemcpyStep_count_succ config state (count + 1)
        hscratch0 hcount (by omega)
      have hnextBound : count + 1 < 2 ^ width := by omega
      have hnextNonzero : BitVec.ofNat width (count + 1) ≠ 0 := by
        intro hz
        have htoNat := congrArg BitVec.toNat hz
        simp only [BitVec.toNat_ofNat, BitVec.toNat_zero] at htoNat
        rw [Nat.mod_eq_of_lt hnextBound] at htoNat
        omega
      have hrest := ih (fuel := fuel + 27)
        (state := stackFrameMemcpyStep config state)
        hscratch0 hscratch1 hscratch2 hscratch3 hstepCount
        hnextNonzero hnextBound (by
          intro address
          exact hheaderDomain address)
      have hrest' :
          evalStackFrameFuelWithCode (fuel + 51 + 28 * count) (fun _ => none)
            (stackFrameMemcpyStep config state) (stackGcMemcpy config) =
          some (.normal (stackFrameMemcpyIter config (count + 1)
            (stackFrameMemcpyStep config state))) := by
        have hfuel : fuel + 27 + (24 + 28 * count) =
            fuel + 51 + 28 * count := by omega
        rw [hfuel] at hrest
        simpa [evalStackFrameFuel] using hrest
      have hstep :=
        evalStackFrameFuel_stackGcMemcpyBody config (fuel + 30 + 28 * count) state
          hscratch0 hscratch1 hscratch2 hscratch3 hheaderDomain
      have hstep' :
          evalStackFrameFuelWithCode (fuel + 50 + 28 * count) (fun _ => none) state
            (stackGcMemcpyBody config) =
          some (.normal (stackFrameMemcpyStep config state)) := by
        have hfuel : fuel + 30 + 28 * count + 20 =
            fuel + 50 + 28 * count := by omega
        rw [hfuel] at hstep
        simpa [evalStackFrameFuel] using hstep
      have hbodyLoop :
          evalStackFrameFuelWithCode (fuel + 51 + 28 * count) (fun _ => none) state
            (.ite .notEqual 0 (.imm 0) (stackGcMemcpyBody config) (.break 0)) =
          some (.normal (stackFrameMemcpyStep config state)) := by
        simp [evalStackFrameFuelWithCode, hcondition, hstep']
      have hrestExpanded :
        ...
        simpa [stackGcMemcpy, stackGcWhile] using hrest'
      have hloop :
          evalStackFrameFuelWithCode (fuel + 24 + 28 * (count + 1))
              (fun _ => none) state
            (.loop (.ite .notEqual 0 (.imm 0) (stackGcMemcpyBody config) (.break 0))) =
          evalStackFrameFuelWithCode (fuel + 51 + 28 * count) (fun _ => none)
            (stackFrameMemcpyStep config state)
            (.loop (.ite .notEqual 0 (.imm 0) (stackGcMemcpyBody config) (.break 0))) := by
        have hfuel : fuel + 24 + 28 * (count + 1) =
            (fuel + 51 + 28 * count) + 1 := by omega
        rw [hfuel]
        simp only [evalStackFrameFuelWithCode]
        rw [hbodyLoop]
      change evalStackFrameFuelWithCode (fuel + 24 + 28 * (count + 1))
          (fun _ => none) state
        (.loop (.ite .notEqual 0 (.imm 0) (stackGcMemcpyBody config) (.break 0))) = _
      simpa [stackFrameMemcpyIter] using hloop.trans hrestExpanded

theorem evalStackFrameFuel_stackGcMoveCopySuffix_iter [NeZero width]
    (config : StackGcConfig) (fuel iterations : Nat)
    (state : StackFrameMachineState width)
    (hscratch0 : config.immediateScratch ≠ 0)
    (hscratch1 : config.immediateScratch ≠ 1)
    (hscratch2 : config.immediateScratch ≠ 2)
    ...
    (hscratch5 : config.immediateScratch ≠ 5)
    (hscratch6 : config.immediateScratch ≠ 6)
    (hcount : state.machine.registers 0 = BitVec.ofNat width (iterations + 1))
    (hcountNonzero : BitVec.ofNat width (iterations + 1) ≠ 0)
    (hbound : iterations + 1 < 2 ^ width)
    (hheaderDomain : ∀ address, state.memoryDomain address = true) :
    evalStackFrameFuel (fuel + 25 + 28 * iterations) state
        (stackGcMoveCopySuffix config) =
      some (.normal (stackGcMoveCopySuffixIterState config iterations state)) := by
  have hstep :=
    evalStackFrameFuel_stackGcMemcpy_iter config fuel iterations state
      hscratch0 hscratch1 hscratch2 hscratch3 hcount hcountNonzero hbound
      hheaderDomain
  have hstep' :
      evalStackFrameFuelWithCode (fuel + 24 + 28 * iterations) (fun _ => none)
        state (stackGcMemcpy config) =
      some (.normal (stackFrameMemcpyIter config (iterations + 1) state)) := by
    simpa [evalStackFrameFuel] using hstep
  simp [stackGcMoveCopySuffix, stackGcMoveCopySuffixIterState,
    evalStackFrameFuel, evalStackFrameFuelWithCode, stackSeq,
    ...
    hheaderDomain, hstep, hstep', hscratch0, hscratch1, hscratch2,
...
theorem evalStackFrameFuel_stackGcMoveCode_copy_iter [NeZero width]
    ...
    (hheaderLength :
      state.machine.memory (stackGcMachineMoveAddress config state) >>>
        ...
        BitVec.ofNat width iterations)
    (hnotforwarding :
      state.machine.memory (stackGcMachineMoveAddress config state) &&&
        BitVec.ofNat width 3 ≠ 0)
    (hcountNonzero : BitVec.ofNat width (iterations + 1) ≠ 0)
    (hcountBound : iterations + 1 < 2 ^ width) :
    evalStackFrameFuel (29 + 28 * iterations) state
      (stackGcMoveCode config) =
      some (.normal (stackGcMoveCopyIterState config iterations state)) := by
  have houter :
  ...
  have hheaderLength' :
      state.machine.memory
        ...
        shiftAmount (BitVec.ofNat width
          (config.wordBits - config.lenSize)) =
        BitVec.ofNat width iterations := by
    simpa [stackGcMachineMoveAddress, wordStackMachineShift,
  ...
  have hcount :
      (stackGcMoveCopyPrefixState config
        (stackGcMoveAddressState config state)).machine.registers 0 =
          BitVec.ofNat width (iterations + 1) := by
  ...
  have hcopySuffix :=
    evalStackFrameFuel_stackGcMoveCopySuffix_iter config 0 iterations
      (stackGcMoveCopyPrefixState config
        (stackGcMoveAddressState config state))
      hscratch0 hscratch1 hscratch2 hscratch3 hscratch4 hscratch5 hscratch6
      hcount hcountNonzero hcountBound hheaderDomain
  have hcopySuffix' :
      evalStackFrameFuelWithCode (25 + 28 * iterations) (fun _ => none)
        (stackGcMoveCopyPrefixState config
          (stackGcMoveAddressState config state))
        (stackGcMoveCopySuffix config) =
      some (.normal (stackGcMoveCopySuffixIterState config iterations
        (stackGcMoveCopyPrefixState config
          (stackGcMoveAddressState config state)))) := by
    simpa [evalStackFrameFuel] using hcopySuffix
  simp [stackGcMoveCode, stackGcMoveCopyIterState, stackSeq,
    evalStackFrameFuel, evalStackFrameFuelWithCode, houter, hinner,
    hprefix, hprefix', hcopyPrefix, hcopyPrefix', hcopySuffix, hcopySuffix']
...
theorem evalStackFrameFuel_stackGcMoveList_one [NeZero width]
    (config : StackGcConfig) (fuel : Nat)
    (state moved : StackFrameMachineState width)
    (hscratch5 : config.immediateScratch ≠ 5)
    (hscratch7 : config.immediateScratch ≠ 7)
    (hscratch8 : config.immediateScratch ≠ 8)
    (hcount : state.machine.registers 7 = BitVec.ofNat width 1)
    (hloadDomain : state.memoryDomain (state.machine.registers 8) = true)
    (hmove :
      evalStackFrameFuel (fuel + 2)
        (stackGcMoveListOnePreState config state)
        (stackGcMoveCode config) =
        some (.normal moved))
    (hpreZero :
      (stackGcMoveListOnePreState config state).machine.registers 7 =
        BitVec.ofNat width 0)
    (hmovedZero : moved.machine.registers 7 = BitVec.ofNat width 0)
    (hmovedAddress :
      moved.machine.registers 8 = state.machine.registers 8) :
    evalStackFrameFuel (fuel + 7) state
        (stackGcMoveListCode config) =
      some (.normal (stackGcMoveListOneState config moved)) := by
  have hmove' :
      evalStackFrameFuelWithCode (fuel + 2) (fun _ => none)
        (stackGcMoveListOnePreState config state)
        (stackGcMoveCode config) =
        some (.normal moved) := by
    simpa [evalStackFrameFuel] using hmove
  have hcondition :
      stackMachineCondition state.machine .notEqual 7 (.imm 0) = true := by
    simp [stackMachineCondition, hcount, NeZero.ne width]
  have hbody :
      evalStackFrameFuelWithCode (fuel + 5) (fun _ => none) state
        (stackSeq [
          .inst (.mem .load 5 8),
        ...
        some (.normal (stackGcMoveListOneState config moved)) := by
    simp [stackGcMoveListOneState, stackGcMoveListOnePreState,
      stackGcSubOne, stackGcAddBytes, stackGcAddImmediate,
      stackGcSubImmediate, stackGcConst, stackGcAdd, stackGcSub,
      stackGcMoveCode, evalStackFrameFuelWithCode, stackSeq,
      stackFrameBasic, stackFrameWriteRegister,
      wordStackMachineWriteRegister, wordStackMachineWriteMemory,
      wordStackMachineBinOp, hloadDomain, hmove', hscratch5, hscratch7,
      hscratch8, Ne.symm hscratch5, Ne.symm hscratch7, Ne.symm hscratch8,
      hmovedAddress]
  have hpostZero :
      (stackGcMoveListOneState config moved).machine.registers 7 =
        BitVec.ofNat width 0 := by
    simp [stackGcMoveListOneState, hmovedZero, hscratch7,
      Ne.symm hscratch7, hpreZero]
  have hbodyLoop :
      evalStackFrameFuelWithCode (fuel + 6) (fun _ => none) state
        (.ite .notEqual 7 (.imm 0)
          ...
          (.break 0)) =
        some (.normal (stackGcMoveListOneState config moved)) := by
    simp [evalStackFrameFuelWithCode, hcondition, hbody]
  have hrest :
      evalStackFrameFuelWithCode (fuel + 6) (fun _ => none)
        (stackGcMoveListOneState config moved)
        (.loop (.ite .notEqual 7 (.imm 0)
          ...
          (.break 0))) =
        some (.normal (stackGcMoveListOneState config moved)) := by
    simp [evalStackFrameFuelWithCode, stackMachineCondition, hpostZero]
  have hloop :
      evalStackFrameFuelWithCode (fuel + 7) (fun _ => none) state
        (.loop (.ite .notEqual 7 (.imm 0)
          (stackSeq [
            .inst (.mem .load 5 8),
            stackGcSubOne config 7,
        ...
        evalStackFrameFuelWithCode (fuel + 6) (fun _ => none)
          ...
          (.loop (.ite .notEqual 7 (.imm 0)
            (stackSeq [
              ...
              stackGcAddBytes config 8])
            (.break 0))) := by
    have hfuel : fuel + 7 = (fuel + 6) + 1 := by omega
    rw [hfuel]
    simp only [evalStackFrameFuelWithCode]
    rw [hbodyLoop]
  change evalStackFrameFuelWithCode (fuel + 7) (fun _ => none) state
      (.loop (.ite .notEqual 7 (.imm 0)
        (stackSeq [
          .inst (.mem .load 5 8),
          stackGcSubOne config 7,
