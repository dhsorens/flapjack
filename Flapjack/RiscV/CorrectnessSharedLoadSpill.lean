...
/-!
# Spill-aware shared-load contracts

Shared loads have the ordinary load frame footprint, but read from the
separate shared-memory component of the executable machine state.
-/

namespace Flapjack.RiscV

theorem evalWordStackMachine_shared_load_preserves_other_value [NeZero width]
    ...
    (heval : (wordStackCompileSharedNat config .load destination (.var address)).bind
  ...
  cases destinationLocation <;> cases addressLocation <;> cases otherLocation <;>
    simp [wordStackCompileSharedNat, wordStackAtomNat, wordStackWritePhysicalNat,
      wordStackReadRegister, wordStackJoin, evalWordStackMachine,
      wordStackLocation, wordStackOffset, hdestination, haddress] at heval
  all_goals
    cases heval
    simp_all [wordStackMachineValue, wordStackLocation, wordStackOffset,
      wordStackMachineWriteRegister, wordStackMachineWriteSlot]

theorem evalWordStackMachine_shared_load_preserves_unrelated_values [NeZero width]
    (config : WordStackConfig) (state final : WordStackMachineState width)
  ...
  have hpreserved := evalWordStackMachine_shared_load_preserves_other_value
