...
/-!
# Spill-aware shift expression contracts

Shifts use the same two temporary registers as binary expressions.  This file
ports their frame non-interference contract so the location-indexed relation
can be carried through the remaining expression cases.
-/

namespace Flapjack.RiscV

theorem evalWordStackMachine_shift_preserves_other_value [NeZero width]
    (config : WordStackConfig) (state final : WordStackMachineState width)
    (operator : Shift) (destination left right other : Nat)
...
theorem evalWordStackMachine_shift_preserves_unrelated_values [NeZero width]
    (config : WordStackConfig) (state final : WordStackMachineState width)
  ...
  have hpreserved := evalWordStackMachine_shift_preserves_other_value
